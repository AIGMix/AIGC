...
namespace aigc
{
    ...
    public:
                                    ...
                                    const std::vector<std::string> &headers)
        {
            ...
            while (true)
            {
                int size = client.Receive(buffer, bufferSize);
                if (size <= 0)
                    break;
                ...
                if (headerOver == false)
                {
                    ...
                    {
                        for (int i = 0; i < receiveHeader.headers.size(); i++)
                        {
            ...
            result.status = receiveHeader.status;
            result.headers = receiveHeader.headers;
            result.cookies = receiveHeader.cookies;
            return result;
        }
                                    ...
                                    const std::vector<std::string> &headers = {})
        {
            ...
            if (method == "GET")
                return GetResponse(url + "?" + body, method, "", headers);
        ...
        {
            Response response = Get(url, parameters, headers);
            if (!response.success)
                return false;
            return JsonHelper::JsonToObject(obj, response.dataStr, keys);
        }
    ...
    private:
        ...
        static bool ParseHeaders(ReceiveHeader &receiveHeader, std::vector<char> &data)
        {
            ...
            while (true)
            {
                const auto iter = std::search(data.begin(), data.end(), std::begin(crlf), std::end(crlf));
                if (iter == data.end())
                    break;
        ...
        static bool ReceiveByChunked(std::vector<char> &retData, std::vector<char> &data, ChunkedParas &chunkedParas)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    //查找此子串的\r\n
                    char crlf[] = {'\r', '\n'};
                    const auto pos = std::search(data.begin(), data.end(), std::begin(crlf), std::end(crlf));
                    if (pos == data.end())
                        break;
